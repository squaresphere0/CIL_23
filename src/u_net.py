...
def patch_accuracy_fn(y_hat, y):
    ...
    h_patches = y.shape[-2] // utils.PATCH_SIZE
    w_patches = y.shape[-1] // utils.PATCH_SIZE
    patches_hat = y_hat.reshape(-1, 1, h_patches, utils.PATCH_SIZE, w_patches,
                                utils.PATCH_SIZE).mean((-1, -3)) > utils.CUTOFF
    patches = y.reshape(-1, 1, h_patches, utils.PATCH_SIZE, w_patches,
                        utils.PATCH_SIZE).mean((-1, -3)) > utils.CUTOFF
